...
namespace UZipDotNet
{
    ...
    {
        public void AddCompressedEntry(CompressedEntry entry)
        {
            WriteFileHeader(entry.OriginalFilePath,entry.FileName);
            _writeFile.Write(entry.CompressedData,0,entry.CompressedData.Length);
            UpdateZipFileHeaderFromEntry(entry);
            

        }
        private void UpdateZipFileHeaderFromEntry(CompressedEntry entry)
        {
            _writeStream.Position =_fileHeaderInfo.FilePos + 14;

            _fileHeaderInfo.CompMethod =entry.CompressionMethod;

            _fileHeaderInfo.FileCRC32 = entry.CRC32;

            _writeFile.Write(entry.CRC32);

            _fileHeaderInfo.CompSize = entry.CompressedSize;

            _writeFile.Write(entry.CompressedSize);

            _fileHeaderInfo.FileSize = entry.OriginalSize;

            _writeFile.Write(entry.OriginalSize);
            ...
            _writeFile.Write(entry.CompressionMethod);

            _writeStream.Position = _writeStream.Length;
        }
        public static long TotalBytes;
        public static long ProcessedBytes;
        public static Action<int> ProgressChanged;
        public static Action<string> SpeedChanged;
        public static Action<string> TimeChanged;
        public static long CurrentFileBytes;
        public static DateTime StartTime;
        public CompressedEntry CompressOnly(string originalFilePath, string fileName,byte[] fileData)
        {
            
            _testStream.SetLength(0);
            _testStream.Position = 0;
           

            _readStream = new MemoryStream(fileData);

            _readFile = new BinaryReader(
                _readStream,
                Encoding.UTF8);
            ...
            return new CompressedEntry
            {
                ...
                CompressedData = _testStream.ToArray(),

                OriginalSize = (uint)fileData.Length,
                CompressedSize = (uint)_testStream.Length,
        ...
        private DeflateZipFile(int compLevel)
    : base(compLevel)
        {
            _memoryMode = true;
        }
        public static DeflateZipFile CreateMemoryCompressor()
        {
            return new DeflateZipFile(DefaultCompression);
        }
        /// <summary>
        /// Create ZIP Archive
        /// </summary>
        ...
        // 这是你为开源库新增的“内存直写”黑科技方法！
        public void CompressFromMemory(string originalFileName, byte[] fileData, string saveName)
        {
            try
            {
                // 1. 让原作者的代码去读原文件的修改时间、日期等属性，生成完美的 ZIP Header
                WriteFileHeader(originalFileName, saveName);

                // 2. 【核心魔法】：瞒天过海！把真正要压缩的数据流强行替换成我们的内存数据！
                _readStream = new MemoryStream(fileData);
                _readFile = new BinaryReader(_readStream, System.Text.Encoding.UTF8);
                _readRemain = (UInt32)_readStream.Length;

                // 每次压缩前必须把校验码清零，否则数据会错乱！
                _readCrc32 = 0;
                ...
                // 5. 【最关键的一步】：把压好的文件正式登记到 ZIP 的中央目录里！
                // 没有这一行，原作者的代码就会把 ZIP 当成空文件删掉！
                UpdateZipFileHeader(_readCrc32);
            }
            catch (Exception)
            {
        ...
        protected override int ReadBytes(byte[] buffer, int pos, int len, out bool endOfFile)
        {
            ...
            long current = Interlocked.Add(ref ProcessedBytes, len);
        ...
        private readonly MemoryStream _testStream = new MemoryStream();
        ...
        {
            _testStream.Write(buffer, pos, len);
        }
